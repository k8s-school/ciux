...
// TODO: Implement something like git diff HEAD^ HEAD  _desk/rootfs/
func HasDiff(repo *git.Repository, pathes []string) (bool, error) {
	// Get the HEAD commit
	head, err := repo.Head()
	if err != nil {
		return false, fmt.Errorf("unable to find head: %v", err)
	}
	// Get the commit object
	commit, err := repo.CommitObject(head.Hash())
	if err != nil {
		return false, fmt.Errorf("unable to find commit: %v", err)
	}
	// Get the parent commit object
	if len(commit.ParentHashes) == 0 {
		slog.Debug("No parent commit found")
		return true, nil
	}
	parent, err := repo.CommitObject(commit.ParentHashes[0])
	if err != nil {
		return false, fmt.Errorf("unable to find parent commit: %v", err)
	}
	...
	for _, fp := range patch.FilePatches() {
		from, to := fp.Files()
		slog.Info("File patch", "from", from.Path(), "to", to.Path())
		if from != nil {
