...
export CIUX_PROMOTED_IMAGE_URL=gitlab-registry.in2p3.fr/astrolabsoftware/fink/fink-broker-noscience:v3.2.0-rc0-49-gf1b49e3
```

`ciux` will store this file in `<PROJECT_DIR>/.ciux.d`.
...
There is a `CIUXCONFIG` file generated by `ciux ignite` for each different label selector. This allow for example to have a different `CIUXCONFIG` for the build or the integration test
